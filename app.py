...
def game1():
    if 'username' in session:
        return render_template('game1.html', username=session['username'])
    return redirect(url_for('login'))
@app.route('/game2')
def game2():
    if 'username' in session:
        return render_template('game2.html', username=session['username'])
    return redirect(url_for('login'))

@app.route('/game3')
def game3():
    if 'username' in session:
        return render_template('game3.html', username=session['username'])
    return redirect(url_for('login'))

@app.route('/game4')
def game4():
    if 'username' in session:
        return render_template('game4.html', username=session['username'])
    return redirect(url_for('login'))
@app.route('/bgame')
def bgame():
    if 'username' in session:
